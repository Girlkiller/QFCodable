...
internal struct QFJSONParser {
    var reader: QFDocumentReader
    var depth: Int = 0

    init(bytes: [UInt8]) {
        self.reader = QFDocumentReader(array: bytes)
    }
    
    static func parse(dictionary: Dictionary<AnyHashable, Any>) -> QFJSONValue {
        var object: [String: QFJSONValue] = [:]
        for (key, value) in dictionary {
            ...
            object[keyString] = parse(value)
        }
        return .object(object)
    }
    
    static func parse<T>(_ value: T) -> QFJSONValue {
        if value is (any Numeric) {
            return .number("\(value)")
        } else if let value = value as? NSNumber {
            return .number(value.stringValue)
        } else if let value = value as? String {
            let numberRegex = #"^-?[0-9]+$"#
            if let _ = value.range(of: numberRegex, options: .regularExpression) {
                return .number(value)
            } else {
                return .string(value)
            }
            ...
            return parse(dictionary: value)
        } else if let value = value as? [Any] {
            return .array(value.map { parse($0) })
        } else {
            return .null
        }
    }

    mutating func parse() throws -> QFJSONValue {
        ...
        while let next = reader.peek(offset: whitespace) {
            ...
            case ._space, ._tab, ._return, ._newline:
                ...
                continue
            default:
                throw QFJSONError.unexpectedCharacter(ascii: next, characterIndex: reader.readerIndex + whitespace)
            }
        }
    ...
    mutating func parseValue() throws -> QFJSONValue {
        var whitespace = 0
        while let byte = reader.peek(offset: whitespace) {
            ...
            case ._space, ._return, ._newline, ._tab:
                ...
                continue
            default:
                throw QFJSONError.unexpectedCharacter(ascii: byte, characterIndex: self.reader.readerIndex)
            }
        }

        throw QFJSONError.unexpectedEndOfFile
    }


    // MARK: - Parse Array -

    mutating func parseArray() throws -> [QFJSONValue] {
        precondition(self.reader.read() == ._openbracket)
        guard self.depth < 512 else {
            throw QFJSONError.tooManyNestedArraysOrDictionaries(characterIndex: self.reader.readerIndex - 1)
        }
        self.depth += 1
        defer { depth -= 1 }
        ...
        while true {
            ...
            case ._comma:
                ...
                continue
            default:
                throw QFJSONError.unexpectedCharacter(ascii: ascii, characterIndex: reader.readerIndex)
            }
        }
    }

    // MARK: - Object parsing -
    mutating func parseObject() throws -> [String: QFJSONValue] {
        ...
        while true {
            ...
            guard colon == ._colon else {
                throw QFJSONError.unexpectedCharacter(ascii: colon, characterIndex: reader.readerIndex)
            }
            reader.moveReaderIndex(forwardBy: 1)
            try reader.consumeWhitespace()
            ...
            case ._comma:
                ...
                continue
            default:
                throw QFJSONError.unexpectedCharacter(ascii: commaOrBrace, characterIndex: reader.readerIndex)
            }
        }
    }
}

extension QFJSONParser {
    struct QFDocumentReader {
        let array: [UInt8]
        ...
        mutating func readBool() throws -> Bool {
            ...
            case UInt8(ascii: "t"):
                guard self.read() == UInt8(ascii: "r"),
                    ...
                    throw QFJSONError.unexpectedCharacter(ascii: self.peek(offset: -1)!, characterIndex: self.readerIndex - 1)
                }
        ...
        private mutating func readUTF8StringTillNextUnescapedQuote() throws -> String {
            ...
            while let byte = peek(offset: copy) {
                ...
                case 0 ... 31:
                    ...
                    throw QFJSONError.unescapedControlCharacterInString(ascii: byte, in: string, index: errorIndex)
                ...
                case UInt8(ascii: "\\"):
                    ...
                    do {
                        ...
                        throw QFJSONError.unexpectedEscapedCharacter(ascii: ascii, in: output!, index: failureIndex)
                    } catch EscapedSequenceError.expectedLowSurrogateUTF8SequenceAfterHighSurrogate(let failureIndex) {
                        output! += try makeString(at: escapedStartIndex ..< self.readerIndex)
                        throw QFJSONError.expectedLowSurrogateUTF8SequenceAfterHighSurrogate(in: output!, index: failureIndex)
                    } catch EscapedSequenceError.couldNotCreateUnicodeScalarFromUInt32(let failureIndex, let unicodeScalarValue) {
                        output! += try makeString(at: escapedStartIndex ..< self.readerIndex)
                        throw QFJSONError.couldNotCreateUnicodeScalarFromUInt32(
        ...
        private func makeString<R: RangeExpression<Int>>(at range: R) throws -> String {
            ...
            guard let str = String(bytes: raw, encoding: .utf8) else {
                throw QFJSONError.invalidUTF8Sequence(Data(raw), characterIndex: range.relative(to: array).lowerBound)
            }
            return str
        }
        ...
        private mutating func parseUnicodeHexSequence() throws -> UInt16 {
            ...
            guard let first = QFDocumentReader.hexAsciiTo4Bits(firstHex),
                  let second = QFDocumentReader.hexAsciiTo4Bits(secondHex),
                  let third = QFDocumentReader.hexAsciiTo4Bits(thirdHex),
                  let forth = QFDocumentReader.hexAsciiTo4Bits(forthHex)
            else {
                let hexString = String(decoding: [firstHex, secondHex, thirdHex, forthHex], as: Unicode.UTF8.self)
                throw QFJSONError.invalidHexDigitSequence(hexString, index: startIndex)
            }
        ...
        private mutating func parseNumber() throws -> String {
            ...
            while let byte = self.peek(offset: numberchars) {
                ...
                case UInt8(ascii: "0"):
                    if hasLeadingZero {
                        throw QFJSONError.numberWithLeadingZero(index: readerIndex + numberchars)
                    }
                ...
                case UInt8(ascii: "."):
                    guard numbersSinceControlChar > 0, pastControlChar == .operand else {
                        throw QFJSONError.unexpectedCharacter(ascii: byte, characterIndex: readerIndex + numberchars)
                    }
